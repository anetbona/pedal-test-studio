...
- `data/audio/<id>/*.wav` — nagrania; `data/events.ndjson` — log zdarzeń (zero PII)
- `public/` — vanilla JS SPA (hash routing): `js/app.js` (routing), `catalog.js` (U1), `configurator.js` (U2–U5), `knobs.js` (animowane gałki), `player.js` (audio), `track.js` (zdarzenia)
...
- Gałki w MVP są **read-only** — animują się do ustawień nagrania; user nimi nie kręci (to V2 z fizycznym rigiem MIDI).
