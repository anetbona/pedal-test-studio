...
- 2026-08-21: UX AUDIO+LAYOUT (decyzje userki): riff w petli i 2x dluzszy (10,2 s, bezszwowe zapetlenie), transport play/stop, kręcenie galka w trakcie grania nie restartuje dzwieku; katalog jako SIATKA zamiast karuzeli; kostka mieści sie w widoku przy wysokosci okna >=600px, max 820px (sztywno powyzej ~1060px), <600px scroll jak wczesniej; pasek sklepow ograniczony do 3 (producent + Thomann + Sweetwater); nagrania wersjonowane ?v=N (cache-busting).
- 2026-08-24: POPRAWKI UX (decyzje userki): podpisy galek nie nachodza na siebie (skalowanie fontu + rozsuwanie na poziomy, min 2px), mniej czarnego tla przy niskich oknach (podglad nie rozciaga sie do wysokosci panelu, sticky), krecenie galka przy ciszy od razu wlacza dzwiek. Serwer dev: no-store dla JS/CSS (unikanie starych wersji przy testach).
